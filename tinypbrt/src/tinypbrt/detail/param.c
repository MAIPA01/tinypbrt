...
#pragma region PARAM
	...
	tpbrt_error_t tpbrt_param_as_strings(const tpbrt_param_t* const param, tpbrt_string_array_t* const out_strings) {
		...
		tpbrt_size_t t_len;
			...
			if (count == 0) {
				out_strings->values = TPBRT_NULL;
				out_strings->count	= 0;
				return TPBRT_ERROR_NONE;
			}

		out_strings->values = malloc(sizeof(tpbrt_string_t) * count);
			if (out_strings->values == TPBRT_NULL) { return TPBRT_ERROR_OUT_OF_MEMORY; }
			...
			while ((curr = tpbrt_next_token(curr, end, &t_len)) != TPBRT_NULL) {
				tpbrt_string_t temp;
					if (!tpbrt_parse_string_token(curr, t_len, &temp)) {
							for (tpbrt_size_t i = 0; i < idx; ++i) { free(out_strings->values[i].chars); }
						free(out_strings->values);
						out_strings->values = TPBRT_NULL;
						out_strings->count	= 0;
						return TPBRT_ERROR_PARSE_STRING;
					}
				tpbrt_copy_string(&out_strings->values[idx], &temp);
	...
	tpbrt_error_t tpbrt_param_as_string(const tpbrt_param_t* const param, tpbrt_string_t* const out_string) {
			...
			if (param->value_type == TPBRT_PARAM_VALUE_TYPE_ARRAY) {
				...
				const tpbrt_error_t err = tpbrt_param_as_strings(param, &strings);
					...
					if (strings.count > 1) {
							for (tpbrt_size_t i = 0; i < strings.count; ++i) { free(strings.values[i].chars); }
						free(strings.values);
						return TPBRT_ERROR_TOO_MANY_VALUES;
					}

				*out_string = strings.values[0];
				free(strings.values);
				return TPBRT_ERROR_NONE;
			}
	...
	tpbrt_error_t tpbrt_param_as_rgbs(const tpbrt_param_t* const param, tpbrt_rgb_array_t* const out_rgbs) {
		...
		out_rgbs->values = malloc(sizeof(tpbrt_rgb_t) * count);
			if (out_rgbs->values == TPBRT_NULL) { return TPBRT_ERROR_OUT_OF_MEMORY; }

		curr			 = param->value.chars;
		tpbrt_size_t idx = 0;
			while ((curr = tpbrt_next_token(curr, end, &t_len)) != TPBRT_NULL) {
					if (!tpbrt_parse_float_token(curr, t_len, &out_rgbs->values[idx].r)) {
						free(out_rgbs->values);
						out_rgbs->values = TPBRT_NULL;
						out_rgbs->count	 = 0;
						return TPBRT_ERROR_PARSE_FLOAT;
					}
				...
				curr = tpbrt_next_token(curr, end, &t_len);
					if (!tpbrt_parse_float_token(curr, t_len, &out_rgbs->values[idx].g)) {
						free(out_rgbs->values);
						out_rgbs->values = TPBRT_NULL;
						out_rgbs->count	 = 0;
						return TPBRT_ERROR_PARSE_FLOAT;
				...
				curr = tpbrt_next_token(curr, end, &t_len);
					if (!tpbrt_parse_float_token(curr, t_len, &out_rgbs->values[idx].b)) {
						free(out_rgbs->values);
						out_rgbs->values = TPBRT_NULL;
						out_rgbs->count	 = 0;
						return TPBRT_ERROR_PARSE_FLOAT;
...
#pragma region PARAMS_LIST
	...
	tpbrt_error_t tpbrt_params_list_get_param(const tpbrt_params_list_t* const params_list,
	  const tpbrt_string_t* const param_name, tpbrt_param_t** const param) {
			if (params_list == TPBRT_NULL || param_name == TPBRT_NULL || param_name->chars == TPBRT_NULL || param == TPBRT_NULL) {
				return TPBRT_ERROR_INVALID_POINTER;
			}
