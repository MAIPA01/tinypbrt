		...
		static const tpbrt_string_t REMAP_ROUGHNESS_STR = TPBRT_STRING("remaproughness");
			...
			default:
				case TPBRT_MATERIAL_TYPE_COATED_DIFFUSE: {
					...
					tpbrt_material_coated_diffuse_params_t* mat_params = &(*material)->coated_diffuse_params;
						...
						if (err == TPBRT_ERROR_NOT_FOUND) {
							err = tpbrt_params_list_get_string(params, &NORMAL_MAP_STR, &texture_name);
								...
								if (err == TPBRT_ERROR_NOT_FOUND) {
									...
									mat_params->bump_map.normal_map = texture_name;
								}
						}
						else {
							err = tpbrt_textures_list_get_opt_texture_handle_of_type(textures, &texture_name,
							  TPBRT_TEXTURE_TYPE_FLOAT, &mat_params->bump_map.displacement);
								if (err != TPBRT_ERROR_NONE && err != TPBRT_ERROR_NOT_FOUND) {
									tpbrt_free_material(material);
									return err;
								}
...
#pragma endregion
					...
					static const tpbrt_float_t REFLECTANCE_DEFAULT = 0.5f;
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->reflectance.f32		   = REFLECTANCE_DEFAULT;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
...
#pragma region ROUGHNESS
					err = tpbrt_params_list_get_string(params, &ROUGHNESS_STR, &texture_name);
						...
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->roughness_params.roughness.f32		  = 0.0f;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
					...
					err = tpbrt_params_list_get_string(params, &U_ROUGHNESS_STR, &texture_name);
						...
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->roughness_params.u_roughness.f32		= 0.0f;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
					...
					err = tpbrt_params_list_get_string(params, &V_ROUGHNESS_STR, &texture_name);
						...
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->roughness_params.v_roughness.f32		= 0.0f;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
...
#pragma endregion
				...
				case TPBRT_MATERIAL_TYPE_COATED_CONDUCTOR: {
					...
					tpbrt_material_coated_conductor_params_t* mat_params = &(*material)->coated_conductor_params;
...
#pragma endregion
					...
					err = tpbrt_params_list_get_spectrum(params, &REFLECTANCE_STR, &mat_params->reflectance);
						if (err != TPBRT_ERROR_NONE && err != TPBRT_ERROR_NOT_FOUND) {
							tpbrt_free_material(material);
							return err;
						}
						...
						if (err == TPBRT_ERROR_NOT_FOUND) {
							err = tpbrt_params_list_get_spectrum(params, &CONDUCTOR_ETA_STR, &mat_params->conductor_eta);
								if (err != TPBRT_ERROR_NONE && err != TPBRT_ERROR_NOT_FOUND) {
									tpbrt_free_material(material);
									return err;
								}
								...
								if (err == TPBRT_ERROR_NOT_FOUND) {
									mat_params->conductor_eta.type	  = TPBRT_SPECTRUM_TYPE_BUILTIN;
									mat_params->conductor_eta.builtin = TPBRT_SPECTRUM_BUILTIN_METAL_CU_ETA;
								}

							err = tpbrt_params_list_get_spectrum(params, &CONDUCTOR_K_STR, &mat_params->conductor_k);
								if (err != TPBRT_ERROR_NONE && err != TPBRT_ERROR_NOT_FOUND) {
									tpbrt_free_material(material);
									return err;
								}
								...
								if (err == TPBRT_ERROR_NOT_FOUND) {
									mat_params->conductor_eta.type	  = TPBRT_SPECTRUM_TYPE_BUILTIN;
									mat_params->conductor_eta.builtin = TPBRT_SPECTRUM_BUILTIN_METAL_CU_K;
								}
						}
...
#pragma endregion
				...
				case TPBRT_MATERIAL_TYPE_CONDUCTOR: {
					...
					tpbrt_material_conductor_params_t* mat_params = &(*material)->conductor_params;
					...
					err = tpbrt_params_list_get_string(params, &REFLECTANCE_STR, &texture_name);
						...
						if (err == TPBRT_ERROR_NOT_FOUND) {
							err = tpbrt_params_list_get_string(params, &ETA_STR, &texture_name);
								...
								if (err == TPBRT_ERROR_NOT_FOUND) {
									mat_params->eta.value_type		 = TPBRT_TEXTURE_HANDLE_VALUE_TYPE_SPECTRUM;
									mat_params->eta.spectrum.type	 = TPBRT_SPECTRUM_TYPE_BUILTIN;
									mat_params->eta.spectrum.builtin = TPBRT_SPECTRUM_BUILTIN_METAL_CU_ETA;
								}
								else {
									err = tpbrt_textures_list_get_opt_texture_handle_of_type(textures, &texture_name,
									  TPBRT_TEXTURE_TYPE_SPECTRUM, &mat_params->eta);
									free(texture_name.chars);
										if (err != TPBRT_ERROR_NONE && err != TPBRT_ERROR_NOT_FOUND) {
											tpbrt_free_material(material);
											return err;
							...
							err = tpbrt_params_list_get_string(params, &K_STR, &texture_name);
								...
								if (err == TPBRT_ERROR_NOT_FOUND) {
									mat_params->k.value_type	   = TPBRT_TEXTURE_HANDLE_VALUE_TYPE_SPECTRUM;
									mat_params->k.spectrum.type	   = TPBRT_SPECTRUM_TYPE_BUILTIN;
									mat_params->k.spectrum.builtin = TPBRT_SPECTRUM_BUILTIN_METAL_CU_K;
								}
								else {
									err = tpbrt_textures_list_get_opt_texture_handle_of_type(textures, &texture_name,
									  TPBRT_TEXTURE_TYPE_SPECTRUM, &mat_params->k);
									free(texture_name.chars);
										if (err != TPBRT_ERROR_NONE && err != TPBRT_ERROR_NOT_FOUND) {
											tpbrt_free_material(material);
											return err;
...
#pragma endregion
				...
				case TPBRT_MATERIAL_TYPE_DIELECTRIC: {
					...
					tpbrt_material_dielectric_params_t* mat_params = &(*material)->dielectric_params;
...
#pragma endregion
					...
					static const tpbrt_float_t ETA_DEFAULT = 1.5f;
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->eta_single.value_type = TPBRT_TEXTURE_HANDLE_VALUE_TYPE_FLOAT;
							mat_params->eta_single.f32		  = ETA_DEFAULT;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
							  TPBRT_TEXTURE_TYPE_FLOAT, ETA_DEFAULT, &mat_params->eta_single);
								...
								if (err == TPBRT_ERROR_INVALID_TEXTURE_TYPE) {
									err = tpbrt_textures_list_get_opt_texture_handle_of_type(textures, &texture_name,
									  TPBRT_TEXTURE_TYPE_SPECTRUM, &mat_params->eta_wavelength);
										if (err != TPBRT_ERROR_NONE) {
											free(texture_name.chars);
											tpbrt_free_material(material);
											return err;
...
#pragma endregion
				...
				case TPBRT_MATERIAL_TYPE_DIFFUSE_TRANSMISSION: {
					...
					tpbrt_material_diffuse_transmission_params_t* mat_params = &(*material)->diffuse_transmission_params;
...
#pragma endregion
					...
					static const tpbrt_float_t TRANSMITTANCE_DEFAULT = 0.25f;
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->transmittance.f32		 = TRANSMITTANCE_DEFAULT;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
				...
				case TPBRT_MATERIAL_TYPE_HAIR: {
					...
					tpbrt_material_hair_params_t* mat_params	= &(*material)->hair_params;
...
#pragma endregion
					...
					static const tpbrt_float_t EUMELANIN_DEFAULT = 1.3f;
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->eumelanin.f32		 = EUMELANIN_DEFAULT;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
					...
					static const tpbrt_float_t ETA_DEFAULT = 1.55f;
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->eta.f32		   = ETA_DEFAULT;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
					...
					static const tpbrt_float_t BETA_M_DEFAULT = 0.3f;
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->beta_m.f32		  = BETA_M_DEFAULT;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
					...
					static const tpbrt_float_t BETA_N_DEFAULT = 0.3f;
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->beta_n.f32		  = BETA_N_DEFAULT;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
					...
					static const tpbrt_float_t ALPHA_DEFAULT = 0.3f;
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->alpha.f32		 = ALPHA_DEFAULT;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
			...
			case TPBRT_MATERIAL_TYPE_INTERFACE:	   break;
				case TPBRT_MATERIAL_TYPE_MEASURED: {
					...
					tpbrt_material_measured_params_t* mat_params = &(*material)->measured_params;
...
#pragma endregion
				...
				case TPBRT_MATERIAL_TYPE_MIX: {
					...
					tpbrt_material_mix_params_t* mat_params	  = &(*material)->mix_params;
					...
					err = tpbrt_params_list_get_strings(params, &MATERIALS_STR, &temp_strings);
						...
						if (temp_strings.count < 2) {
								if (temp_strings.values != TPBRT_NULL) {
										for (tpbrt_size_t i = 0; i < temp_strings.count; ++i) {
											free(temp_strings.values[i].chars);
										}
									free(temp_strings.values);
								}
							tpbrt_free_material(material);
						...
						if (temp_strings.count > 2) {
								for (tpbrt_size_t i = 0; i < temp_strings.count; ++i) { free(temp_strings.values[i].chars); }
							free(temp_strings.values);
							tpbrt_free_material(material);
						...
						for (tpbrt_size_t i = 0; i < temp_strings.count; ++i) {
							err = tpbrt_materials_list_get_material_handle(materials, &temp_strings.values[i],
							  &mat_params->materials[i]);
								if (err != TPBRT_ERROR_NONE) {
										for (tpbrt_size_t j = 0; j < temp_strings.count; ++j) {
											free(temp_strings.values[j].chars);
										}
									free(temp_strings.values);
									tpbrt_free_material(material);
									return err;
					...
					static const tpbrt_float_t AMOUNT_DEFAULT = 0.5f;
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->amount.f32		  = AMOUNT_DEFAULT;
						}
						else {
							err = tpbrt_textures_list_get_texture_handle_of_type(textures, &texture_name,
				...
				case TPBRT_MATERIAL_TYPE_SUBSURFACE: {
					...
					tpbrt_material_subsurface_params_t* mat_params = &(*material)->subsurface_params;
...
#pragma endregion
					...
					static const tpbrt_rgb_t SIGMA_A_DEFAULT = (tpbrt_rgb_t) { .r = 0.0011f, .g = 0.0024f, .b = 0.014f };
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->sigma_a.spectrum.type = TPBRT_SPECTRUM_TYPE_RGB;
							mat_params->sigma_a.spectrum.rgb  = SIGMA_A_DEFAULT;
						}
						else {
							err = tpbrt_textures_list_get_opt_texture_handle_of_type(textures, &texture_name,
							  TPBRT_TEXTURE_TYPE_SPECTRUM, &mat_params->sigma_a);
					...
					static const tpbrt_rgb_t SIGMA_S_DEFAULT = (tpbrt_rgb_t) { .r = 2.55f, .g = 3.12f, .b = 3.77f };
						if (err == TPBRT_ERROR_NOT_FOUND) {
							...
							mat_params->sigma_s.spectrum.type = TPBRT_SPECTRUM_TYPE_RGB;
							mat_params->sigma_s.spectrum.rgb  = SIGMA_S_DEFAULT;
						}
						else {
							err = tpbrt_textures_list_get_opt_texture_handle_of_type(textures, &texture_name,
...
#pragma endregion
				...
				case TPBRT_MATERIAL_TYPE_THIN_DIELECTRIC: {
					tpbrt_material_thin_dielectric_params_t* mat_params = &(*material)->thin_dielectric_params;
...
#pragma endregion
	...
	tpbrt_error_t tpbrt_create_named_material(const tpbrt_string_t* const name, const tpbrt_params_list_t* const params,
	  const tpbrt_textures_list_t* const textures, const tpbrt_materials_list_t* const materials,
	  tpbrt_material_t** const material) {
			if (name == TPBRT_NULL || name->chars == TPBRT_NULL || params == TPBRT_NULL || textures == TPBRT_NULL ||
				materials == TPBRT_NULL || material == TPBRT_NULL) {
				return TPBRT_ERROR_INVALID_POINTER;
			}
	...
	void tpbrt_free_material(tpbrt_material_t** const material) {
			...
			if ((*material)->name.chars != TPBRT_NULL) { free((*material)->name.chars); }

			switch ((*material)->type) {
				case TPBRT_MATERIAL_TYPE_COATED_DIFFUSE: {
					const tpbrt_material_coated_diffuse_params_t* params = &(*material)->coated_diffuse_params;
						if (params->bump_map.type == TPBRT_MATERIAL_BUMP_MAP_TYPE_NORMAL_MAP &&
							params->bump_map.normal_map.chars != TPBRT_NULL) {
							free(params->bump_map.normal_map.chars);
						}
				}
				case TPBRT_MATERIAL_TYPE_COATED_CONDUCTOR: {
					const tpbrt_material_coated_conductor_params_t* params = &(*material)->coated_conductor_params;
						if (params->bump_map.type == TPBRT_MATERIAL_BUMP_MAP_TYPE_NORMAL_MAP &&
							params->bump_map.normal_map.chars != TPBRT_NULL) {
							free(params->bump_map.normal_map.chars);
						}
				...
				case TPBRT_MATERIAL_TYPE_CONDUCTOR: {
					const tpbrt_material_conductor_params_t* params = &(*material)->conductor_params;
						if (params->bump_map.type == TPBRT_MATERIAL_BUMP_MAP_TYPE_NORMAL_MAP &&
							params->bump_map.normal_map.chars != TPBRT_NULL) {
							free(params->bump_map.normal_map.chars);
						}
				...
				case TPBRT_MATERIAL_TYPE_DIELECTRIC: {
					const tpbrt_material_dielectric_params_t* params = &(*material)->dielectric_params;
						if (params->bump_map.type == TPBRT_MATERIAL_BUMP_MAP_TYPE_NORMAL_MAP &&
							params->bump_map.normal_map.chars != TPBRT_NULL) {
							free(params->bump_map.normal_map.chars);
						}
				...
				case TPBRT_MATERIAL_TYPE_DIFFUSE: {
					const tpbrt_material_diffuse_params_t* params = &(*material)->diffuse_params;
						if (params->bump_map.type == TPBRT_MATERIAL_BUMP_MAP_TYPE_NORMAL_MAP &&
							params->bump_map.normal_map.chars != TPBRT_NULL) {
							free(params->bump_map.normal_map.chars);
						}
				...
				case TPBRT_MATERIAL_TYPE_DIFFUSE_TRANSMISSION: {
					const tpbrt_material_diffuse_transmission_params_t* params = &(*material)->diffuse_transmission_params;
						if (params->bump_map.type == TPBRT_MATERIAL_BUMP_MAP_TYPE_NORMAL_MAP &&
							params->bump_map.normal_map.chars != TPBRT_NULL) {
							free(params->bump_map.normal_map.chars);
						}
				...
				case TPBRT_MATERIAL_TYPE_HAIR: {
					const tpbrt_material_hair_params_t* params = &(*material)->hair_params;
						if (params->bump_map.type == TPBRT_MATERIAL_BUMP_MAP_TYPE_NORMAL_MAP &&
							params->bump_map.normal_map.chars != TPBRT_NULL) {
							free(params->bump_map.normal_map.chars);
						}
				...
				case TPBRT_MATERIAL_TYPE_MEASURED: {
					const tpbrt_material_measured_params_t* params = &(*material)->measured_params;
						if (params->bump_map.type == TPBRT_MATERIAL_BUMP_MAP_TYPE_NORMAL_MAP &&
							params->bump_map.normal_map.chars != TPBRT_NULL) {
							free(params->bump_map.normal_map.chars);
						}
				...
				case TPBRT_MATERIAL_TYPE_SUBSURFACE: {
					const tpbrt_material_subsurface_params_t* params = &(*material)->subsurface_params;
						if (params->bump_map.type == TPBRT_MATERIAL_BUMP_MAP_TYPE_NORMAL_MAP &&
							params->bump_map.normal_map.chars != TPBRT_NULL) {
							free(params->bump_map.normal_map.chars);
						}

						if (params->name.chars != TPBRT_NULL) { free(params->name.chars); }
				}
				case TPBRT_MATERIAL_TYPE_THIN_DIELECTRIC: {
					const tpbrt_material_thin_dielectric_params_t* params = &(*material)->thin_dielectric_params;
						if (params->bump_map.type == TPBRT_MATERIAL_BUMP_MAP_TYPE_NORMAL_MAP &&
							params->bump_map.normal_map.chars != TPBRT_NULL) {
							free(params->bump_map.normal_map.chars);
						}
...
#pragma region MATERIALS_LIST
	...
	tpbrt_error_t tpbrt_materials_list_get_material_by_name(const tpbrt_materials_list_t* const materials_list,
	  const tpbrt_string_t* const material_name, const tpbrt_material_t** const material) {
			if (materials_list == TPBRT_NULL || material_name == TPBRT_NULL || material_name->chars == TPBRT_NULL ||
				material == TPBRT_NULL) {
				return TPBRT_ERROR_INVALID_POINTER;
			}
			...
			for (tpbrt_size_t i = 0; i < materials_list->count; i++) {
					if (materials_list->materials[i].name.size == material_name->size &&
						strncmp(materials_list->materials[i].name.chars, material_name->chars, material_name->size) == 0) {
