...
extern "C" {
    ...
    typedef struct {
        ...
        union {
            // Include, Import
            struct { tpbrt_string_t path; } include_import;

            // Option
            struct { tpbrt_param_t param; } option;
            ...
            struct { tpbrt_string_t type_name; } single_string;

            // CoordinateSystem, CoordSysTransform, PixelFilter, NamedMaterial,
            // ObjectBegin, ObjectInstance
            struct { tpbrt_string_t name; } named_entity;

            // Translate, Scale
            struct { tpbrt_float_t v[3]; } vector3;

            // Rotate
            struct { tpbrt_float_t angle; tpbrt_float_t v[3]; } rotate;

            // LookAt
            struct { tpbrt_float_t eye[3]; tpbrt_float_t look_at[3]; tpbrt_float_t up[3]; } look_at;

            // Transform, ConcatTransform
            struct { tpbrt_float_t m[16]; } transform;

            // TransformTimes
            struct { tpbrt_float_t start; tpbrt_float_t end; } transform_times;
    ...
    void tpbrt_parser_init(tpbrt_parser_t* parser, const tpbrt_char_t* str, tpbrt_size_t length);
